<?php
    ...
    if (empty($social_media_id) || empty($access_token)){
        ...
        if ($nonce != null){
            ...
            // If nonce is inserted successfully, attempt to register user.
            if ($nonce_data->insert_nonce($auth_nonce)){
                ...
                // If user is found, return an error within the response
                if ($user != null){
                    $response->error = true;
                    $response->message = "That account has already been registered within our system.";
                } else {
                    // Attempt to insert user into the system. If insert fails, return an error within the response. 
                    if ($user_data->insert_social_media_user($social_media_id,$social_media_platform)){
                        // Attempt to find a user in the users table using social media id and social media platform. If successful, set user session. Otherwise, return an error within the response.
                        $user = $user_data->get_user_by_social_media_id_and_social_media_platform($social_media_id,$social_media_platform);
                        if ($user != null){
